' ## Windows SAP Manager Uxtrace Version 0.94 
' ## Created by ZWA (Orsyp) 2008/03/04
' ## Minor bug fix
' ## 
' ## Windows SAP Manager Uxtrace Version 0.93 
' ## Created by ZWA (Orsyp) 2008/02/29
' ## Collecting the *.trc files from 64 bit Windows
' ## Added check to see if the trace is run in a $U env instead
' ## 
' ## Windows SAP Manager Uxtrace Version 0.9 
' ## Created by ZWA (Orsyp) 2007/09/05
' ##
...
' ## Minor enhancements
' ## Windows Uxtrace Version 4.3.2
' ## Modified by ZWA (Orsyp) 2006/06/27
' ## Minor enhancements
' ## Get BVI data files and customer created views
'''''''''''
''Constant
'''''''''''
...
If batch_mode="no" then
	...
	file_level=inputbox("If you know the data file collecting level (0 - 1), please specify it here", "The files to be collected", default_files, 2000, 2000)
	if not isnumeric(file_level) then 'If the input is not a number, quit!
		msgbox("Please try again and specify 0 - 1 for date file colleting.")
		wscript.quit
	end if
	if file_level <0 or file_level >1 then 
		msgbox("Please try again and specify 0 - 1 for date file colleting.")
		wscript.quit
	end if
	...
	If colNamedArguments.Exists("file_level") Then
		...
		if not isnumeric(file_level) then 
			wscript.echo "Please try again and specify 0 - 1 for date file colleting."
			wscript.quit
		end if
		if file_level <0 or file_level >1 then 
			wscript.echo "Please try again and specify 0 - 1 for date file colleting."
...
Elseif objFSO.FileExists("..\uxsetenv.bat") then
	...
	if batch_mode="no" then
		msgbox("Please unpack the uxtrace procedure under the 'mgr' directory of the SAP manager.")
	else
...
objShell.Run("%comspec% /k .\uxtrace_step1.bat  "& file_level &"  "& env_path &""), 1, True
