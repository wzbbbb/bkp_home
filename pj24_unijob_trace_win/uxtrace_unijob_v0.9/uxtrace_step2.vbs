...
win_trace_ver="UNIJOB Uxtrace 0.9.0"
ver_short="v09"
...
UXDIR_ROOT=getvar_def("UNI_DIR_ROOT")
UXEXE=getvar_def("UXEXE")
UXMGR=getvar_def("UXMGR")
'UXDAP=var_def("UXDAP")
'UXDIN=var_def("UXDIN")
'UXDSI=var_def("UXDSI")
UXDEX=getvar_def("UXDEX")
'UXLAP=var_def("UXLAP")
'UXLIN=var_def("UXLIN")
'UXLSI=var_def("UXLSI")
UXLEX=getvar_def("UXLEX")
'UXPAP=var_def("UXPAP")
'UXPIN=var_def("UXPIN")
'UXPSI=var_def("UXPSI")
UXPEX=getvar_def("UXPEX")
U_TMP_PATH=getvar_def("U_TMP_PATH")
...
S_SOCIETE=getvar_def("S_SOCIETE")
...
trace_log "################################################################## "
trace_log "***********                                            *********** "
trace_log "***********     Welcome to UNIJOB Uxtrace 0.9.0        *********** "
...
'ir_tc UXPIN, "UXPIN",dls_dir &"UXPIN_cr.txt"
'ir_tc UXPSI, "UXPSI",dls_dir &"UXPSI_cr.txt"
...
cmd_ts "unichecklm.bat", U_DIR_ANALYSIS &"\analyse.txt"
cmd_ts UXEXE & "\getvar UNI_VERSION", U_DIR_ANALYSIS &"\analyse.txt"
...
'cp UXMGR & "\*.bat", files_dir
'cp UXMGR & "\*.dta", files_dir
'cp UXMGR & "\*.key", files_dir
'cp UXMGR & "\*.xml", files_dir
'cmd_ "if exist %uxmgr%\uxstartup* copy %uxmgr%\uxstartup* " & files_dir
cp UXMGR & "\u_batch_cmd", files_dir
'cmd_ "if exist %UXMGR%\*.bat copy %uxmgr%\*.bat " & files_dir
'cmd_ "if exist %UXMGR%\*.dat copy %uxmgr%\*.dat " & files_dir
'cmd_ "if exist %UXMGR%\*.key copy %uxmgr%\*.key " & files_dir
'cmd_ "if exist %UXMGR%\*.xml copy %uxmgr%\*.xml " & files_dir
...
for ind_ =3 to 3 
	...
	Loop
	'Set objExecObject = objShell.Exec("cmd /c dir /B *IU_PUR*")
	'Do While Not objExecObject.StdOut.AtEndOfStream
	'    log_name_ = objExecObject.StdOut.ReadLine()
	'    cmd_ "copy " & log_name_ & " " & log_dir &   log_dir_var(ind_,0) &"_"& log_name_
	'Loop
	'Set objExecObject = objShell.Exec("cmd /c dir /B *IU_RTS*")
	'Do While Not objExecObject.StdOut.AtEndOfStream
	'    log_name_ = objExecObject.StdOut.ReadLine()
	'    cmd_ "copy " & log_name_ & " " & log_dir &   log_dir_var(ind_,0) &"_"& log_name_
	'Loop
...
'''''''''''$U admin listing
'wscript.echo "Getting UNIJOB Admin information ... "
'if instr(target , "O")=0 then
'	trace_log "Getting UNIJOB Admin information ... "
...
if instr(target ,"S")<>0 then
	...
	'trace_log "Getting UNIJOB registry ..."
