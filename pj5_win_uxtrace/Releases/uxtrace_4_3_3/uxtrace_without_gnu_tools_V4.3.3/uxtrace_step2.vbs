...
trace_log "Getting Dollar Universe directory listings for data files ..."
...
win_trace_ver="Windows Uxtrace 4.3.3"
ver_short="v40"
...
trace_log "################################################################## "
trace_log "***********                                            *********** "
trace_log "***********     Welcome to Windows Uxtrace 4.3.3       *********** "
...
'xxx
trace_log "Getting Dollar Universe directory/file, creation time ..."
...
trace_log "Getting Dollar Universe directory/file, last access time ..."
...
trace_log "Getting Dollar Universe directory/file, last written time ..."
...
trace_log "Getting Dollar Universe directory/file, access right ..."
...
cmd_ "if exist %UXMGR%\u_fali01*.* copy %uxmgr%\u_fali01*.* " & files_dir
...
'''''''''''Working on the LOGs
cmd_ "if exist " & U_LOG_FILE & " copy " & U_LOG_FILE &" "& log_dir
Select Case area 
    case "A" log_d=UXLAP 
    case "I" log_d=UXLIN
    case "S" log_d=UXLSI 
...
for ind_ =0 to 3 
	...
	'    case "3" area_="X" 
	'End Select	
	'log_list = Array("uxiotrns.log", S_SOCIETE & "_CAL"& area_ &".log",S_SOCIETE &"_ORD"& area_ &".log",S_SOCIETE &"_ECH"&area_ &".log",S_SOCIETE &"_SUR"&area_ &".log","uxcdjsrv.log","uchglibeltypuproc.log","uxnotify.log","u_purge.log","uxbvssrv.log","uxcmdsrv.log","*IU_PUR*","*IU_RTS*")
	log_list = Array("uxiotrns.log", "*_CAL*.log","*_ORD*.log","*_ECH*.log","*_SUR*.log","uxcdjsrv.log","uchglibeltypuproc.log","uxnotify.log","u_purge.log","uxbvssrv.log","uxcmdsrv.log","*IU_PUR*","*IU_RTS*")
	trace_log log_dir_var(ind_,0) & " is: " & log_dir_var(ind_,1)

	Set objShell = WScript.CreateObject("WScript.Shell")
	objShell.CurrentDirectory = log_dir_var(ind_,1)
	for each log_name_ in log_list

	Set objExecObject = objShell.Exec("cmd /c dir /B " & log_name_) 'to find the good .log file name, then copy
	Do While Not objExecObject.StdOut.AtEndOfStream
		log_name_ = objExecObject.StdOut.ReadLine()
		'wscript.echo "log file name: "& log_dir_var(ind_,1) &"\" & log_name_
		cmd_ "echo #TSP# #BEG# " & U_DIR_ANALYSIS & ">> " &  log_dir_var(ind_,1) &"\"&log_name_
		cmd_ "copy " & log_dir_var(ind_,1) &"\" &log_name_ &" " & log_dir & log_dir_var(ind_,0) &"_"& log_name_
	'    cmd_ "echo #TSP# #BEG# " & U_DIR_ANALYSIS & ">> " &  log_name_
	'    cmd_ "copy " & log_name_ & " " & log_dir &   log_dir_var(ind_,0) &"_"& log_name_
	Loop
		'cmd_ "echo #TSP# #BEG# " & U_DIR_ANALYSIS & ">> " &  log_name_
		'cmd_ "copy " & log_name_ & " " & log_dir &   log_dir_var(ind_,0) &"_"& log_name_
		'cmd_ "if exist " & log_dir_var(ind_,1) &"\" & log_name_ & "echo #TSP# #BEG# " & U_DIR_ANALYSIS & ">> " &  log_dir_var(ind_,1) &"\"&log_name_
		'cmd_ "if exist " & log_dir_var(ind_,1) &"\" & log_name_ & " copy " & log_dir_var(ind_,1) &"\" &log_name_ &" " & log_dir & log_dir_var(ind_,0) &"_"& log_name_
	next

' The following is the original way, which uses VBS, but apparently doesnot work when there are 6k files.
'	Set FileList = obj_cimv2.ExecQuery _
