...
#
# 10/17/2008
# added the fmbvm and fcml for razfic 
#
# multi_dump 1.0
# by ZWA 
...
DU_ENV ()
{
	...
	do
		...
		do
			#l_dir_lngth=`echo ${l_dir_tc} | wc -c` update for V600 VVV
			#l_dir_lngth=`echo ${l_dir_tc} | wc -c`
			l_dir_lngth=`echo ${l_dir_tc} |cut -f1 -d"_" |wc -c`
			real_soc=`echo ${l_dir_tc} |cut -f1 -d"_" `
...
DUMP () {
	...
	if [ ! -f ${g_clean_script} ]; then
		echo "$UXEXE/uxrazfic u_fmcm60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmcx60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmer60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmev60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmfu60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmhs60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmlc60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmlp60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmph60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmpi60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmsb60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmsp60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmtp60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fseu60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fecl60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fecd60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fppf60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fmpf60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fbvi60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fbvm60 ${l_area}" >> ${g_clean_script}
		echo "$UXEXE/uxrazfic u_fcml60 ${l_area}" >> ${g_clean_script}
	...
	do
		...
		if [ ${l_skp} = off ]; then
			...
			if [ -f ${l_dta_dir}/${l_file} ]; then
				echo "$UXEXE/uxrazfic ${l_file_base} ${l_area}" >> ${g_clean_script}
			...
			if [ -f ${UXDIR_ROOT}/${l_file} ]; then
				...
				# No Need to remove the .dta?
				#echo "$UXEXE/uxrazfic ${l_file_base}" >> ${g_clean_script}
			fi
			l_flag_reorg=on
		else
	...
	#[ ${l_flag_reorg:-off} = on ] && $UXMGR/ux_vrf_rgz_rst_area ${l_area}
	 [ ${l_flag_reorg:-off} = on ] && $UXEXE/unireorg 
	...
	case ${l_rest_ans:-y} in
		...
		[yY] ) ${UXMGR}/uxstartup;l_du_str_flg=on;;
		[nN] ) ;;
		* ) ;;
	esac
