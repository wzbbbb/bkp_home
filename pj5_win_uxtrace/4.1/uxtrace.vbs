...
' ##
' ## The ISMEMBER.EXE used in this script is created 
...
If batch_mode="no" then
	...
	target=inputbox("Please specify the tracing target:" & vbCrLf & "C : for Dollar Universe Configuration "& vbCrLf &  "S : for OS "& vbCrLf & "CS : for All (both Configuration and OS)" & vbCrLf & "O : consider the Dollar Universe Compay shutdown", "Tracing target",  default_target , 2000, 2000)
	target=UCase(target)
	if target ="SC" then target = "CS" end if
	if target <> "S" and target <> "C" and target <> "CS" and target <> "O"  then
		msgbox("Please try again and specify C, S, CS or O for tracing target.")
		wscript.quit
	end if
else
