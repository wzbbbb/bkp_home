...
If batch_mode="no" then
	...
	file_level=inputbox("If you know the data file collecting level (0 - 2), please specify it here", "The files to be collected", default_files, 2000, 2000)
	if not isnumeric(file_level) then 'If the input is not a number, quit!
		msgbox("Please try again and specify 0 - 2 for date file colleting.")
		wscript.quit
	end if
	if file_level <0 or file_level >2 then 
		msgbox("Please try again and specify 0 - 2 for date file colleting.")
		wscript.quit
	end if
	...
	If colNamedArguments.Exists("file_level") Then
		...
		if not isnumeric(file_level) then 
			wscript.echo "Please try again and specify 0 - 2 for date file colleting."
			wscript.quit
		end if
		if file_level <0 or file_level >2 then 
			wscript.echo "Please try again and specify 0 - 2 for date file colleting."
