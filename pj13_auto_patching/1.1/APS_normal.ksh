...
stop_du() {
	...
	ps -ef |grep  ${S_SOCIETE} |grep -v grep |grep -v APS_normal.ksh
	# kill the remaining process
	if [ $? -eq 0  ] ; then  
		# get the process IDs
		list=`ps -ef|grep  ${S_SOCIETE}|grep -v grep|grep -v APS_normal.ksh|tr -s ' ' ' '  |cut -f2 -d" "`
		kill ${list}
	fi
	# check again, find if any processes are not stopped 
	ps -ef |grep  ${S_SOCIETE} |grep -v grep |grep -v APS_normal.ksh
	# kill the remaining process
	if [ $? -eq 0  ] ; then  
		# get the process IDs
